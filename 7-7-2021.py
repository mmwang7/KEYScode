...
def eventID_inputs (df):
    ...
    var = df.index[~df['INPUT'].str.contains(':')].tolist()
    for val in var:
...
# Small file testing
small = pd.read_csv('./2016/PMC4207242-arizona-out.tsv', encoding='utf-8', sep='\t')
small = clean(small)
small[25:35]
...
total = pd.read_csv('./bigfile.csv', encoding='utf-8')
# total
n = len(pd.unique(total['SEEN IN']))
print(n)
...
# 0 = Everything else
conds = [total['EVENT LABEL'].eq('Activation (Positive)'),
         total['EVENT LABEL'].eq('Activation (Negative)')]
choices = [1, -1]
total['INTERACTION VALUE'] = np.select(conds, choices, default=0)
total

# Checking dupicate rows based on OUTPUT, and CONTROLLER column
dups_df = total[total.duplicated(['OUTPUT', 'CONTROLLER'], keep=False)]
